...
public class UpdateEventService {
    ...
    public void persistLatestStories() {
        log.info("Updating latest stories");
        Update updates = hnUtil.fetchUpdatedStories();
        if(Objects.nonNull(updates)){
            List<Long> updatedEvents = updates.getItems();

            List<Story> stories = updatedEvents.stream()
        ...
        log.info("Updated latest stories");
    }
